...
sig_genes_df_pl = subset(sig_genes_df_pl, abs(Log2FoldChange) > 0.3)
sig_genes_df_pl_up = subset(sig_genes_df_pl, Log2FoldChange > 0.3)
sig_genes_df_pl_down = subset(sig_genes_df_pl, Log2FoldChange < -0.3)
...
ids_pl <-bitr(names(original_gene_list_pl), fromType = "UNIPROT",
              toType = "ENTREZID", OrgDb=organism) # remove duplicate IDS (here I use "ENSEMBL", but it should be whatever was selected as keyType)
