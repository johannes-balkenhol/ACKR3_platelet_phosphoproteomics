...
#prep data
dataset <- SummarizedExperiment::assay(ppe, "normalised")  #this is the same as norm_intensity
...
##specific kinases
kinases = c("PRKACA", "PRKG1", "PRKAA1", "CDK1", "MAPKAPK2", "AKT1", "MTOR", "SRC", "CDK2")
...
kinase_function <- function(x) {
  ...
  tiff(filename = paste0("analysis/Heatmap/Heatmap_", x, "_targets_sign.tiff", sep =""), #for kinases
  ...
  tiff(filename = paste0("analysis/Heatmap/Heatmap_", x, "_targets_sign2.tiff", sep =""), #for kinases
       #tiff(filename = paste0("analysis/Heatmap/Heatmap_", protein, "_sign.tiff", sep =""), #for proteins
       width = 10 * 300, 
       height = 10 * 300,
       res = 300,
...
##specific proteins
protein_list <- list("VASP", "PDE3A", "SRC")

protein_function <- function(x) {
  targets_norm_abundance <- dataset_df[str_detect(row.names(dataset_df), paste(protein, ";", sep = "")), 1:53]
  targets_norm_abundance <-targets_norm_abundance[order(row.names(targets_norm_abundance)), ]
  targets_norm_abundance <- as.matrix(targets_norm_abundance)
...
tiff(filename = paste0("analysis/Heatmap/Heatmap_", x, "_sign2.tiff", sep =""), #for proteins
       width = 10 * 300, 
       height = 10 * 300,
       res = 300,
       compression = "lzw")
