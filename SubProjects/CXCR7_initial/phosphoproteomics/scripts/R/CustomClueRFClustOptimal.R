...
#' 
#' 
clustOptimalÖO <- function(clueObj, rep=5, user.maxK=NULL, effectiveSize=NULL, pvalueCutoff=0.05, visualize=NULL, universe=NULL, mfrow = c(1, 1)) {
  ...
  } else if (visualize == "scaled") {
    fuzzPlotScaled(clueObj$Tc, clustObj = bst.cobj, mfrow)
  } else if (visualize == "log") {
    fuzzPlotLog(clueObj$Tc, clustObj = bst.cobj, mfrow)
  } else {
    stop("Invalid value for 'visualize'. Please use NULL, 'scaled', or 'log'.")
  }
