...
#exclude phosphopeptides that were mapped to more than one uniprot ids from further downstream analyses
not.uniq = read.delim(file = "data/processed_data/not_unique_ids.txt", sep = "\t")

top.10 = top.10[!(row.names(top.10) %in% not.uniq[,1]), ]
top.30 = top.30[!(row.names(top.30) %in% not.uniq[,1]), ]
top.60 = top.60[!(row.names(top.60) %in% not.uniq[,1]), ]
top.300 = top.300[!(row.names(top.300) %in% not.uniq[,1]), ]
top.600 = top.600[!(row.names(top.600) %in% not.uniq[,1]), ]
top.900 = top.900[!(row.names(top.900) %in% not.uniq[,1]), ]
top.1800 = top.1800[!(row.names(top.1800) %in% not.uniq[,1]), ]
...
DE2.RUV #[1]  132  176  283  747 1058  942 1164
##after filtering nonunique ids out: [1]  130  176  283  738 1052  937 1152
