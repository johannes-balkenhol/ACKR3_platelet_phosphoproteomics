...
df<- as.data.frame(rownames(intensity_data))
rownames(df) <- rownames(intensity_data)
# split the new variable into two pieces, delete the pattern (the :), and keep both pieces   
newdf <- stringr::str_split_fixed(df$`rownames(intensity_data)`, ";", 5) 
colnames(newdf) <-  c("uniprot_id", "genename", "site", "sequence", "id")
rownames(newdf) <-  rownames(df)
annotation_data <- as.data.frame(newdf)
...
## define header
donor_nr <- gsub("Donor", "",sapply(strsplit(colnames(intensity_data), "_"), "[[", 3))
time_point <- sapply(strsplit(colnames(intensity_data), "_"), "[[", 1)
timepoint_fac <- as.factor(time_point)
condition <- sapply(strsplit(colnames(intensity_data), "_"), "[[", 2)

sample_metadata <- data.frame(colnames(intensity_data), condition, donor_nr, paste0(condition, "_", time_point))
