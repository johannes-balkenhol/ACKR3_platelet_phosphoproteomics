...
assign(paste0("gse.go", names_input[[i]]), gse.go)
...
gseaplot(gse.r, geneSetID = "R-HSA-1660661")
dev.new()
...
dev.new()
dotplot(gse.r, showCategory=10, split=".sign") + facet_grid(.~.sign)


#### enrichment map
gse_pw <- pairwise_termsim(gse.r)
dev.new()
...
#### Cnetplot
dev.new()
cnetplot(gse.r, categorySize="pvalue", foldChange=gene_list_uniprot, showCategory = 5)

#### Ridgeplot
dev.new()
#png(file=paste0("../analysis/gsea_analysis/", "gse.r.10_ridgeplot_",".png"), width = 7, height = 10, units = 'in', res = 600)
ridgeplot(gse.go,  showCategory = 20,core_enrichment = TRUE,orderBy = "pvalue", label_format = 40, decreasing = TRUE) 
