...
all_miss <- apply(data, 2, function(x) all(is.na(x)))
na_columns <- names(all_miss[all_miss>0])   
na_col_nums <- c(which(colnames(data) == na_columns[1]), which(colnames(data) == na_columns[2]))
data <- data[-na_col_nums]
...
tiff("../analysis/PCA/correlation_raw_style2.tiff", 
