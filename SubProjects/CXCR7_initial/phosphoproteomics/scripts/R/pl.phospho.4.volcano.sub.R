...
for (i in 1:length(input)) {
  ...
  label <- sapply(strsplit(rownames(top.x.b), ";"), 
  ...
  #dev.new()
  png(file=paste0("../analysis/Volcano_plots/", names_input[i],".png"), width = 1200, height = 1000, bg = "white")
  ...
  print(EnhancedVolcano(top.x.b,
    ...
    theme(
      axis.title = element_text(size =60), # Adjust size of axis titles
      axis.text.y = element_text(size = 50),  # Ensure consistency with axisLabSize
      axis.text.x = element_text(
        size = 50,                 # Adjust font size
        margin = margin(t = 10)    # Move labels away from the axis (t = top margin in points)
      )
      ))
  #	+ coord_flip()
  #i = i + 1
