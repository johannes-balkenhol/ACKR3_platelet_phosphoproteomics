...
#make heatmap of empiricals --> first heatmap of everything, then take empiricals
top.norm_intensity <- norm_intensity
...
data_subset_norm_input <- data_subset_norm[rownames(data_subset_norm) %in% empirical_topall,]
data_subset_norm_input2 <- data_subset_norm[rownames(data_subset_norm) %in% top.rownames,]
...
dev.new()
pheatmap(mat = data_subset_norm_input, 
