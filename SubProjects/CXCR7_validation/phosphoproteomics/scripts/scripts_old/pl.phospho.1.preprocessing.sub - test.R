...
### go to script folder
setwd("C:/Users/Ozge/Nextcloud/Phosphoproteom/phosphoproteom validation") ##ÖO folder structure should fit.
#setwd("D:/Eigene Datein/Phosphoproteom/phosphoproteom validation/scripts/")
setwd("F:/Masterarbeit_BioWis/Phosphoproteomics/Phosphoproteom validation") ## JB private Laptop
setwd("C:/Users/job37yv/Research/Phospho/phospho validation/") ## JB uni laptop
...
df_pl <- raw_abundance3_log 
df_pl$PSite <- Site(ppe0) <- sapply(strsplit(rownames(ppe0), ";"), "[[", 3)
df_pl$Uniprot_ID <- sapply(strsplit(rownames(ppe0), ";"), "[[", 1)
df_pl$Name <- sapply(strsplit(rownames(ppe0), ";"), "[[", 2)
#df_pl$id <- rownames(ppe0)
...
df_pl2 <- df_pl %>%
  group_by(PSite,Uniprot_ID,Name) %>%
