...
for (i in 1:length(input)) {
    ...
    df_pl = input[[i]]
    df_pl = subset(df_pl, select = c(id, AveExpr, logFC, adj.P.Val))
    #rownames(df_pl) <- NULL
    df_pl <- df_pl %>%
      separate(id, c("Protein", "Gene", "Residue.Both", "Peptide", "delete"), ";")
    ## get rid of site without annotation
    #df_pl = df_pl[-which(df_pl$Residue.Both == ""),]
    df_pl = df_pl[, c(1,2,6,7,8,3,5)]
    ...
    df_pl_temp <- plyr::ddply(df_pl, .(uniprot_id, PSite), dplyr::summarise,
                name = max(name),
    ...
    ## collapse same site by choosing the one with the max(abs(logFC))
    df_pl2 <- df_pl %>%
      group_by(uniprot_id, PSite) %>%
      slice(which.max(abs(logFC))) %>%
      ungroup()
    
    df_pl2 <- as.data.frame(df_pl2)
    rownames(df_pl2) <- df_pl2$ID
    df_pl2 <- df_pl2[c(1,2,3,4,5,6)]
    ...
    top.collapse <- top.collapse[order(top.collapse$PValue),]
